#!/bin/bash
# usage: gdsArea0.rb [-n] <gdsFileIn> <xmlReportOut> [ <gdsFileOut> ]
#
...
x=%{
  ...
  exec klayout -e -b -r "$0[drc]" -rd tag="$_M0"
...
# argv.each{ |v| puts v }
  ...
  OptionParser.new do |opts|
    opts.banner = usage
    opts.on("-m MARKER_DB_PATH", "--marker_db_path=MARKER_DB_PATH", "XML marker database report path") do |m|
      o[:marker_db_path] = m
    end
