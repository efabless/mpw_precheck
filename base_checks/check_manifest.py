...
def check_manifest(target_path, output_file,lc=logging_controller(default_logger_path,default_target_path),call_path='/usr/local/bin/base_checks', git_url):
...
def check_manifests(target_path, output_file,lc=logging_controller(default_logger_path,default_target_path),call_path='/usr/local/bin/base_checks'):
    check, reason, fail_lines = check_manifest(target_path+'/verilog/rtl',str(output_file)+'.rtl.log',lc,call_path,rtl_manifest_git_url)
    if check:
        check, reason, fail_lines = check_manifest(target_path+'/maglef',str(output_file)+'.maglef.log',lc,call_path,maglef_manifest_git_url)
        if check:
            check, reason, fail_lines = check_manifest(target_path+'/mag',str(output_file)+'.mag.log',lc,call_path,mag_manifest_git_url)    
